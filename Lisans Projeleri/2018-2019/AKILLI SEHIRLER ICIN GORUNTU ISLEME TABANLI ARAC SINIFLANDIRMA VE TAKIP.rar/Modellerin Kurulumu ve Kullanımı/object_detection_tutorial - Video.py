...
import imageio                                                              # İmageio kütüphanesi dahil edildi.

reader = imageio.get_reader("Video/1.mp4")                                  # Video okundu.

fps = reader.get_meta_data()["fps"]                                         # Giriş ve çıkış videosunun eşit olması için fps bilgisi alındı.
writer = imageio.get_writer("Video/faster_rcnn_resnet101_coco_output.mp4", fps = fps)
                                                                            # Video yazdırıldı ve fps'ler eşitlendi.
...
with tf.Session(graph=detection_graph) as sess:
    for i,image_np in enumerate(reader):
      ...
      print(i)                                      # Kaçıncı frame de olduğumuzu anlamak için kullanıldı.
    writer.close()                                  # Yazma kapandı.
