...
cap = cv2.VideoCapture(0)                                          # Kameradan görüntü yakalandı.
...
with tf.Session(graph = detection_graph) as sess:
    while True :
        ret, image_np = cap.read()                                          # Yakalanan görüntü okundu.
        ...
        cv2.imshow("Video",image_np)                                        # Görüntü ekerana getirildi.

        if cv2.waitKey(1) & 0xFF == ord("q") :                              # Eğer q tuşuna basılırsa,
            cv2.destroyAllWindows()                                         # Bütün pencereler kapatılır.
